...
var sproto = (function () {
    ...
    function toword(stream, startIndex = 0) {
        return (stream[startIndex+0] & 0xff) | ((stream[startIndex+1] & 0xff) << 8);
    }

    function todword(stream, startIndex = 0) {
        return (
            ((stream[startIndex+0] & 0xff) |
                ((stream[startIndex+1] & 0xff) << 8) |
                ((stream[startIndex+2] & 0xff) << 16) |
                ((stream[startIndex+3] & 0xff) << 24)) >>>
            0
        );
    }

    function count_array(stream, startIndex=0) {
        var length = todword(stream, startIndex);
        var n = 0;
        startIndex += SIZEOF_LENGTH;
        var nsz;
        while (length > 0) {
            if (length < SIZEOF_LENGTH) {
                return -1;
            }
            nsz = todword(stream, startIndex);
            nsz += SIZEOF_LENGTH;
            if (nsz > length) {
                return -1;
            }
    ...
    function struct_field(stream, sz, startIndex=0) {
        var field, fn, header, i;
        if (sz < SIZEOF_LENGTH) {
            return -1;
        }

        fn = toword(stream, startIndex);
        ...
        var fieldStartIndex = startIndex + SIZEOF_HEADER;
        sz -= header;
        startIndex += header;
        // stream = stream.slice(header);
        for (i = 0; i < fn; i++) {
            var value = toword(stream, fieldStartIndex + i * SIZEOF_FIELD + SIZEOF_HEADER);
            var dsz;
            if (value != 0) {
                continue;
            }
            ...
            dsz = todword(stream, startIndex);
            if (sz < SIZEOF_LENGTH + dsz) {
                return -1;
            }

            startIndex += SIZEOF_LENGTH + dsz;
    ...
    // stream 是arraybuffer
    function import_string(s, stream, startIndex = 0) {
        var sz = todword(stream, startIndex);
        var buffer = "";
        var arr = stream.slice(startIndex + SIZEOF_LENGTH, startIndex + SIZEOF_LENGTH + sz);
    ...
    function import_field(s, f, stream, startIndex=0) {
        var sz, result, fn;
        var array = 0;
        var tag = -1;
        f.tag = -1;
        ...
        sz = todword(stream, startIndex);
        startIndex += SIZEOF_LENGTH;
        // stream = stream.slice(SIZEOF_LENGTH);
        result = stream.slice(startIndex + sz);
        fn = struct_field(stream, sz, startIndex);
        if (fn < 0) return null;

        startIndex += SIZEOF_HEADER;
        // stream = stream.slice(SIZEOF_HEADER);
        for (var i = 0; i < fn; i++) {
            var value;
            ++tag;
            value = toword(stream, startIndex+SIZEOF_FIELD * i);
            if (value & (1 != 0)) {
            ...
            if (tag == 0) {
                ...
                f.name = import_string(s, stream, startIndex+fn * SIZEOF_FIELD);
                continue;
            }
    ...
    function import_type(s, t, stream,startIndex=0) {
        ...
        for (var i = 0; i < fn * SIZEOF_FIELD; i += SIZEOF_FIELD) {
            var v = toword(stream, startIndex+SIZEOF_HEADER + i);
            if (v != 0) return null;
        }
        ...
        t.maxn = 0;
        t.f = null;
        startIndex += SIZEOF_HEADER + fn * SIZEOF_FIELD;
        // stream = stream.slice(SIZEOF_HEADER + fn * SIZEOF_FIELD);
        t.name = import_string(s, stream, startIndex);

        if (fn == 1) {
            return result;
        }

        startIndex += todword(stream, startIndex) + SIZEOF_LENGTH;
        // stream = stream.slice(todword(stream) + SIZEOF_LENGTH);
        n = count_array(stream, startIndex);
        if (n < 0) {
            return null;
        }
        ...
        for (var i = 0; i < n; i++) {
            ...
            t.f[i] = new Object();
            var f = t.f[i];
            stream = import_field(s, f, stream, startIndex);
            startIndex = 0;
            if (stream == null) {
                return null;
            }
    ...
    */
    function import_protocol(s, p, stream, startIndex = 0) {
        ...
        for (var i = 0; i < fn; i++, tag++) {
            var value = toword(stream, startIndex + SIZEOF_FIELD * i);
            if (value & (1 != 0)) {
            ...
            switch (i) {
                case 0:
                    ...
                    p.name = import_string(s, stream, startIndex + SIZEOF_FIELD * fn);
                    break;
                case 1:
                    if (value < 0) {
                        return null;
    ...
    function create_from_bundle(s, stream, sz, startIndex = 0) {
        var content, typedata,typedataIndex, protocoldata, protocoldataIndex;
        var fn = struct_field(stream, sz, startIndex);
        if (fn < 0 || fn > 2) return null;
        startIndex += SIZEOF_HEADER;
        // stream = stream.slice(SIZEOF_HEADER);
        let contentIndex = startIndex + fn * SIZEOF_FIELD;
        // content = stream.slice(fn * SIZEOF_FIELD);

        for (var i = 0; i < fn; i++) {
            var value = toword(stream, startIndex + i * SIZEOF_FIELD);
            if (value != 0) {
                return null;
            }

            var n = count_array(stream, contentIndex);
            if (n < 0) {
                return null;
            }

            if (i == 0) {
                typedata = stream.slice(contentIndex + SIZEOF_LENGTH);
                s.type_n = n;
                s.type = new Array();
            } else {
                protocoldataIndex = contentIndex + SIZEOF_LENGTH;
                // protocoldata = content.slice(SIZEOF_LENGTH);
                s.protocol_n = n;
                s.proto = new Array();
            }
            contentIndex += todword(stream, contentIndex) + SIZEOF_LENGTH;
        ...
        for (var i = 0; i < s.protocol_n; i++) {
            ...
            stream = import_protocol(s, s.proto[i], stream, protocoldataIndex);
            protocoldataIndex = 0;
            if (stream == null) {
                return null;
            }
    ...
    function decode_array_object(cb, args, stream, sz, startIndex = 0) {
        var hsz;
        var index = 1;
        while (sz > 0) {
            if (sz < SIZEOF_LENGTH) {
            ...
            args.index = index;
            args.value = stream.slice(startIndex);
            args.length = hsz;
            if (cb(args) != 0) {
                return -1;
            }
    ...
    function decode_array(cb, args, stream, startIndex=0) {
        var sz = todword(stream, startIndex);
        var type = args.type;
        if (sz == 0) {
            args.index = -1;
        ...
        switch (type) {
            case SPROTO_TINTEGER:
                ...
                if (len == 4) {
                    ...
                    for (var i = 0; i < Math.floor(sz / 4); i++) {
                        var value = expand64(todword(stream, startIndex + i * 4));
                        args.index = i + 1;
                        args.value = value;
                        args.length = 8;
                        cb(args);
                    ...
                    for (var i = 0; i < Math.floor(sz / 8); i++) {
                        var low = todword(stream, startIndex +(i * 8));
                        var hi = todword(stream, startIndex +(i * 8 + 4));
            ...
            case SPROTO_TSTRING:
            case SPROTO_TSTRUCT:
                return decode_array_object(cb, args, stream, sz, startIndex);
            default:
                return -1;
        }
        return 0;
    ...
    (t.pack = function (inbuf) {
        ...
        (t.createNew = function (binsch) {
            ...
            function sproto_decode(st, data, size, cb, ud) {
                var args = new Object();
                var total = size;
                var stream,streamIndex, datastream, datastreamIndex, fn, tag;
                if (size < SIZEOF_HEADER) return -1;
                stream = data;
                fn = toword(stream);
                streamIndex = SIZEOF_HEADER;
                // stream = stream.slice(SIZEOF_HEADER);
                size -= SIZEOF_HEADER;
                if (size < fn * SIZEOF_FIELD) return -1;
                datastreamIndex = streamIndex + fn * SIZEOF_FIELD;
                ...
                for (var i = 0; i < fn; i++) {
                    ...
                    var f = null;
                    var value = toword(stream, streamIndex+(i * SIZEOF_FIELD));
                    ++tag;
                    if (value & (1 != 0)) {
                        tag += Math.floor(value / 2);
                        continue;
                    ...
                    value = Math.floor(value / 2) - 1;
                    currentdataIndex = datastreamIndex;
                    if (value < 0) {
                        var sz;
                        if (size < SIZEOF_LENGTH) {
                            return -1;
                        }
                        sz = todword(stream, datastreamIndex);
                        if (size < sz + SIZEOF_LENGTH) {
                            return -1;
                        }
                        datastreamIndex += sz + SIZEOF_LENGTH;
                    ...
                    if (value < 0) {
                        if ((f.type & SPROTO_TARRAY) != 0) {
                            if (decode_array(cb, args, stream, currentdataIndex)) {
                                return -1;
                            }
                        } else {
                            switch (f.type) {
                                case SPROTO_TINTEGER: {
                                    var sz = todword(stream, currentdataIndex);
                                    if (sz == 4) {
                                        var v = expand64(todword(stream, currentdataIndex+(SIZEOF_LENGTH)));
                                        args.value = v;
                                        args.length = 8;
                                        cb(args);
                                    } else if (sz != 8) {
                                        ...
                                        var low = todword(stream, currentdataIndex+(SIZEOF_LENGTH));
                                        var hi = todword(stream, currentdataIndex+(SIZEOF_LENGTH + 4));
                                ...
                                case SPROTO_TSTRUCT: {
                                    var sz = todword(stream, currentdataIndex);
                                    args.value = stream.slice(currentdataIndex+(SIZEOF_LENGTH));
                                    args.length = sz;
                                    if (cb(args) != 0) {
                                        return -1;
                                    }
            ...
            function queryproto(sp, pname) {
                ...
                if (v === null || v === undefined) {
                    ...
                    if(!ret){
                        console.error("not find proto",pname)
                        return
                    }
                    var tag = ret.ret1;
                    var req = ret.request;
            ...
            host.attach = function (sp) {
                this.attachsp = sp;
                var self = this;
                return (name, args, session, clientSenderSession) => {
